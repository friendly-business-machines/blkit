...
func New(cfg Config) *Store {
	if cfg.DSN == "" {
		panic(fmt.Errorf("mysql state store: DSN is required"))
	}
	...
	if !prefixPattern.MatchString(prefix) {
		panic(fmt.Errorf("mysql state store: invalid TablePrefix %q", cfg.TablePrefix))
	}
	parsed, err := gomysql.ParseDSN(cfg.DSN)
	if err != nil {
		panic(fmt.Errorf("mysql state store: parse DSN: %w", err))
	}
	parsed.ParseTime = true
	parsed.Loc = time.UTC
	db, err := sql.Open("mysql", parsed.FormatDSN())
	if err != nil {
		panic(fmt.Errorf("mysql state store: open: %w", err))
	}
	...
	s.schemaOnce.Do(func() {
		...
		for _, stmt := range stmts {
			if _, err := s.db.Exec(stmt); err != nil {
				s.schemaErr = fmt.Errorf("mysql state store: schema: %w", err)
				return
			}
		}
	})
