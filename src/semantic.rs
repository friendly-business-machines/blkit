...
fn check_block(
    ...
    for stmt in body {
        match stmt {
            Stmt::Return(value) => {
                let actual = infer(value, Some(output), input, input_type, program)?;
                if &actual != output {
                    return Err(format!("return type mismatch: expected {output}, got {actual}"));
                }
            }
            Stmt::If(condition, yes, no) => {
                let actual = infer(condition, None, input, input_type, program)?;
...
fn infer(
    expr: &Expr, expected: Option<&Type>, input: &str, input_type: &Type, program: &Program,
) -> Result<Type, String> {
    use Expr::*;
    ...
    match expr {
        ...
        Name(name) if name == input => Ok(input_type.clone()),
        Name(name) => Err(format!("unknown name: {name}")),
        Field(base, field) => {
            ...
            let ty = infer(base, None, input, input_type, program)?;
            if let Type::Named(name) = ty
        ...
        List(elements) => {
            ...
            let element_type = element_type.or_else(|| {
                elements.first().and_then(|element| infer(element, None, input, input_type, program).ok())
            }).ok_or("cannot infer empty list type")?;
            for element in elements {
                let actual = infer(element, Some(&element_type), input, input_type, program)?;
        ...
        Not(value) => {
            let ty = infer(value, None, input, input_type, program)?;
            if ty != named("Bool") {
        ...
        Binary(left, op, right) => {
            let lhs = if matches!(left.as_ref(), List(elements) if elements.is_empty()) {
                let other = infer(right, None, input, input_type, program)?;
                infer(left, Some(&other), input, input_type, program)?
            } else {
                infer(left, None, input, input_type, program)?
            };
            let rhs = infer(right, Some(&lhs), input, input_type, program)?;
            if lhs != rhs {
