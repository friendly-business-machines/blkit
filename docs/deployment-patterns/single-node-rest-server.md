# Single-Node REST Server

> One binary that serves clients *and* runs processes — a REST server with an
> embedded worker. Self-contained, with no separate worker fleet to operate.
...
!!! warning "Placeholder — pattern in progress"
    ...
    `specs/rest/` and `specs/worker/`.

## What this pattern is

A single binary runs a producer — a [REST server](../architecture/worker-pools.md)
(`blkit/restserver`) — with an **embedded worker** in the same process. The server
accepts submissions over HTTP and streams events; the embedded worker consumes them
and drives each process to completion. Both share one broker and are governed by one
lifecycle: cancelling the server's context drains the worker too.

For an agent-facing binary that exposes processes as MCP tools instead of an HTTP
API, see the [MCP Server for AI Agents](mcp-server.md) pattern.

## When to use it

- **Small deployments** that want an HTTP API without operating a separate
  worker tier.
...
- A REST server started with its `Run(...)` entry point and the
