...
MariaDB server started on demand**: the test spins up a throwaway MariaDB container
with [testcontainers-go](https://golang.testcontainers.org/) and removes it
afterwards, so `go test` needs only a working Docker daemon. Setting
`BLKIT_TEST_MARIADB_DSN` overrides this and points the suite at an already-running
