...
- Each subsequent row is a rule: rule index, then (if `showRuleIDs=true`) the rule id in a `rule-id` column, then input entries rendered via `ToMarkdown()` (with `"-"` for omitted/wildcard inputs), then the `█` separator, then output values (with `""` for omitted outputs).
