...
description: A DecisionNode generic over a single-field outputs struct — evaluates a single bl.BlExpr body whose result is bound to the outputs struct's field
targets:
...
A `LiteralExpression` is a `DecisionNode` defined by a single `bl.BlExpr` body. When evaluated, the body is computed against the input variables, and the result is the value of the node's single output.
...
`NewLiteralExpression` enforces that `Outputs` has exactly one exported field. The body's declared type (the static type of `opts.Body` as a `bl.BlExpr`) must match that field's `Bl*` type; otherwise `DecisionDefinitionError` is raised. The constructor populates the single field with a typed handle so downstream nodes can reference this node's output as `node.Outputs.FieldName`.
...
Here `loanAmount` and `rate` are typed `bl.BlNumber` handles drawn from earlier nodes' `.Outputs.X` fields (or DecisionTask-level inputs). The body composes them with `Bl` operators; the resulting `bl.BlExpr` is typed `bl.BlNumber`, matching the outputs struct's single field.
...
`score` is a typed `bl.BlNumber` handle from an upstream node (e.g., `creditCheck.Outputs.Score`).
...
`eligibility.Outputs.Eligibility` is the typed `bl.BlString` handle exposed by the `eligibility` `DecisionTable` node from its `EligibilityOutputs` struct.

---

## Type Checking

The outputs struct's single field declares the expected output type. The body's compile-time type (e.g., `bl.BlNumber`, `bl.BlString`) must match that field. A mismatch is a `DecisionDefinitionError` at construction time. A runtime body value that disagrees with its compile-time type (which should not happen for type-safe `Bl*` expressions, but may for raw values) produces a `bl.TypeError` at evaluation time.

---

## Markdown Rendering

`ToMarkdown()` returns a markdown string showing the node name, the referenced input variables, and the body rendered via `bl.BlExpr.String()`.
...
- A `Body` that evaluates to `bl.BlNull` is a valid result.
- A `Body` that evaluates to `bl.BlNull` is treated as compatible with any output type.
