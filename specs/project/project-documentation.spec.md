...
1. **Business overview** — a plain-English presentation of the process drawn from the corresponding spec: the overview, input/trigger data, the decision rules or process steps, outcomes, and worked examples. The page should reproduce the spec's natural sections so it stands alone on the documentation site, rather than only linking out. Decision-style and process-style examples will not share identical headings. A process-style example includes a Mermaid business process map before the implementation; the map presents activities, decisions, and outcomes in business terms rather than as process-API constructors or implementation node identifiers.
2. **Implementation** — progressive, runnable Go for every part of the example that current blkit APIs can express. Functionality requiring an unfinished module has no Go block yet and does not prevent available code on the same page from being compiled and tested. When an implementation is partial for this reason, the section opens with a `warning` admonition titled **Implementation in progress** that says the code is subject to change and distinguishes what is implemented from the process behaviour that is not yet available. The limitation is stated once in the admonition rather than repeated after the code.
