...
2. `Evaluate()` walks forward from token positions, processing non-task nodes synchronously:
   ...
   - **Suspend events** — `SuspendForDuration`, `SuspendUntilDatetime`, `SuspendUntilMessage` record a suspension step and cause `Evaluate()` to return with `Status: SUSPENDED`. **Pause events** (`PauseForDuration`, `PauseUntilMessage`) block the goroutine in-process; status remains `RUNNING`.
...
A process can suspend mid-evaluation when it reaches a `SuspendForDuration`, `SuspendUntilDatetime`, or `SuspendUntilMessage` event node (see [event-nodes.spec.md](event-nodes.spec.md)). When this happens, `Evaluate()` returns with `Status: SUSPENDED` rather than continuing to walk the graph. The token rests at the suspending node and is preserved in `result.History` so that a later `Evaluate()` call (passing the persisted `Context` and `History`) can resume from the same position.

`SUSPENDED` is a non-terminal status. The caller is responsible for arranging resumption — typically by persisting `result.History` and scheduling a follow-up `Evaluate()` when the awaited event arrives. The blkit FAAS handlers (see [../faas/overview.spec.md](../faas/overview.spec.md)) implement this pattern by publishing a continuation command via the configured `BrokerGateway` whenever the process suspends. For message-driven resumption (`SuspendUntilMessage`), see the Message Delivery section in [event-nodes.spec.md](event-nodes.spec.md).
